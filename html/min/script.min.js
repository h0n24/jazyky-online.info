...
function getTableItems(a,e){var o=[];$.each(vocabulary[e],function(a,e){e=e.toString().replace(",",", "),o.push("<tr><th>"+a+"</th><td>"+e+"</td></tr>")}),$(a).html(o),$(a).prev("h2").html(e),$("#set-slovicka li.active").removeClass("active"),$("#set-slovicka a[data-category='"+e+"']").parent().addClass("active")}function checkAds(){""==$(".adsense ins.adsbygoogle").html()&&$("#hlaska_po_blokaci").modal()}$("body").scrollspy({target:"#nav-scrollpsy",offset:"-100"}),$(this).scrollspy("refresh"),$(function(){$("a[href*=#]:not([href=#])").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var a=$(this.hash);if(a=a.length?a:$("[name="+this.hash.slice(1)+"]"),a.length)return $("html,body").animate({scrollTop:a.offset().top},1e3),!1}})}),$("table.table-floathead").floatThead();try{var url="min/vocabulary-simple.min.json";$.ajax({url:url,dataType:"json",success:function(a){console.log("Načten [json] "+url+" + uložen do localStorage"),localStorage.setItem("vocabulary",JSON.stringify(a));var e=new Date;localStorage.setItem("vocabulary-updated",JSON.stringify(e))},error:function(a,e,o){console.warn("chyba v [jsonu]: "+url+", stav: "+e+", chybu háže: "+o)}});var vocabulary=JSON.parse(localStorage.getItem("vocabulary")),category,categories=[];$.each(vocabulary,function(a){categories.push('<li><a href="#scroll" data-category="'+a+'">'+a+"</a></li>")}),$("#set-slovicka").html(categories),getTableItems("#get-slovicka","abstract"),$("#set-slovicka a").on("click",function(){category=$(this).data("category"),getTableItems("#get-slovicka",category),$(".affix").removeClass("affix").addClass("affix-top").removeAttr("style")})}catch(err){console.warn("Problém s načítáním jsonu. Pracujete na localhostu?")}$("#learning #input_word").focus();var word="zelená";$("#learning #input_word").on("keyup",function(){var a=$(this).val();word=word.toLowerCase(),a=a.toLowerCase(),console.log(a),""==a?($("#learning .form-group").removeClass("has-success").removeClass("has-error"),$("#learning .form-control-feedback").removeClass("glyphicon-remove").removeClass("glyphicon-ok")):word.indexOf(a)>=0?($("#learning .form-group").removeClass("has-error").addClass("has-success"),$(".form-control-feedback").removeClass("glyphicon-remove").addClass("glyphicon-ok"),word==a&&($("#learning .form-group").removeClass("has-success"),$("#input_word").attr("disabled",!0),$("#next_word").removeClass("btn-default").addClass("btn-primary"),$("#next_word").focus())):($("#learning .form-group").removeClass("has-success").addClass("has-error"),$(".form-control-feedback").removeClass("glyphicon-ok").addClass("glyphicon-remove"))}),$(document).ready(function(){setTimeout("checkAds();",3e3)});