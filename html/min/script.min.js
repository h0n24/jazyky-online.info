...
function getTableItems(e,a){var o=[];$.each(vocabulary[a],function(e,a){a=a.toString().replace(",",", "),o.push("<tr><th>"+e+"</th><td>"+a+"</td></tr>")}),$(e).html(o),$(e).prev("h2").html(a),$("#set-slovicka li.active").removeClass("active"),$("#set-slovicka a[data-category='"+a+"']").parent().addClass("active")}function checkAds(){""==$(".adsense ins.adsbygoogle").html()&&$("#hlaska_po_blokaci").modal()}$("body").scrollspy({target:"#nav-scrollpsy",offset:"-100"}),$(this).scrollspy("refresh"),$(function(){$("a[href*=#]:not([href=#])").click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var e=$(this.hash);if(e=e.length?e:$("[name="+this.hash.slice(1)+"]"),e.length)return $("html,body").animate({scrollTop:e.offset().top},1e3),!1}})}),$("table.table-floathead").floatThead();try{var url="min/vocabulary-simple.min.json";$.ajax({url:url,dataType:"json",success:function(e){console.log("Načten [json] "+url+" + uložen do localStorage"),localStorage.setItem("vocabulary",JSON.stringify(e));var a=new Date;localStorage.setItem("vocabulary-updated",JSON.stringify(a))},error:function(){console.warn("[json] "+url+" nebyl správně načten.")}})}catch(err){console.warn("Problém s načítáním jsonu. Pracujete na localhostu? Err_code: "+err)}try{var vocabulary=JSON.parse(localStorage.getItem("vocabulary")),render_table_categories=[];$.each(vocabulary,function(e){render_table_categories.push('<li><a href="#scroll" data-category="'+e+'">'+e+"</a></li>")}),$("#set-slovicka").html(render_table_categories),getTableItems("#get-slovicka","abstract"),$("#set-slovicka a").on("click",function(){var e=$(this).data("category");getTableItems("#get-slovicka",e),$(".affix").removeClass("affix").addClass("affix-top").removeAttr("style")})}catch(err){console.warn("Problém s načítáním localStorage. "+err)}$("#learning #input_word").focus(),$("#learning #input_word").on("keyup",function(){var e=$(this).val().toLowerCase(),a=["zelený","zelená","zelené"];if(""==e)$("#learning .form-group").removeClass("has-success").removeClass("has-error"),$("#learning .form-control-feedback").removeClass("glyphicon-remove").removeClass("glyphicon-ok");else for(var o=0;o<a.length;o++)if(a[o]=a[o].toLowerCase(),a[o].indexOf(e)>=0){if($("#learning .form-group").removeClass("has-error").addClass("has-success"),$(".form-control-feedback").removeClass("glyphicon-remove").addClass("glyphicon-ok"),a[o]==e){$("#learning .form-group").removeClass("has-success"),$("#input_word").attr("disabled",!0),$("#next_word").removeClass("btn-default").addClass("btn-primary"),$("#next_word").focus();break}}else $("#learning .form-group").removeClass("has-success").addClass("has-error"),$(".form-control-feedback").removeClass("glyphicon-ok").addClass("glyphicon-remove")}),$(document).ready(function(){setTimeout("checkAds();",3e3)});